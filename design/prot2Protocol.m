function protocol_obj = prot2Protocol(old_prot_struct)
    ...
    if isfield(old_prot_struct, 'OPTIONS')
        for fieldNameCell = fieldnames(old_prot_struct.OPTIONS)'
            fname = char(fieldNameCell);
            try
        ...
        for m_idx = 1:length(mod_names)
            ...
            if isfield(mod_data, 'data') && ~isempty(mod_data.data)
                ...
                for row_idx = 1:size(data_table, 1)
                    ...
                    userData = [];
                    if ~(isempty(buddy) || (isstring(buddy) && strlength(buddy) == 0))
                        buddyName = strtrim(char(string(buddy)));
                        if ~isempty(buddyName)
                            userData = struct('Pair', buddyName);
                        end
                    end
                    
                    % Add parameter to Software interface
                    try
                        protocol_obj.addParameter('Software', param_name, val, Access=access, UserData=userData);
                    catch ME
