...
- **Values:** `1` (up), `-1` (down), `NaN` (first trial or neutral)
...
**Purpose:** Create a new Staircase object and attach it to the experiment's event system.

**Parameters:**
- `RUNTIME` — The Runtime object containing trial data and event infrastructure
...
**Details:**
- The constructor automatically attaches a listener to `RUNTIME.HELPER`'s `NewData` event
- When new trials arrive, `update_data()` is called automatically
- The destructor properly cleans up the listener, so the object can be safely deleted
