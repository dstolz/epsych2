...
- **Load** — apply the selected phase: parameter values from the file are written to the live parameters and synchronized into the trial table, and a protocol recompile is scheduled for the next trial boundary so the phase's trial structure (value lists, expressions) takes effect, not just its current values. **During a run (Record or Preview), a load that changed any parameter then regenerates the pending trial**, exactly as the [Regenerate Trial](gui_RegenerateTrial.md) button does: the trial the rig is holding was dispatched with the previous phase's values, so it is dispatched again with the new ones instead of waiting for the next trial. Like the button, **this interrupts a trial in progress** without asking, and the trial counter does not move; the session notes record it (`Regenerated trial 12 (row 3): phase "..." loaded`). A load that changed nothing, or one made while the session is idle or stopped, does not regenerate. The session-control buttons (**Deliver Trials**, **Reminder**, **Shape**, **Observe**, **Pellet**, **Trough**) are **not** touched by a load — see below. While a phase is selected but not yet applied, the button reads **"Load \*"** in an amber color to flag that the selection is only staged; pressing it (or returning the dropdown to `< Select Phase >`) restores the default "Load" look.
...
- After a load that changed something, `regenerateAfterLoad_` re-dispatches each affected box's pending trial through `gui.components.RegenerateTrial.redispatch` — the button's own dispatch, log, and note, without its arming. It runs only while the run mode is Record or Preview, which the component **follows** from `ModeChange` (a listener made in the constructor) rather than reads: a behavior GUI is built before `RunExpt` broadcasts the mode, and asking the interfaces would be a device round trip. A runtime with no `EventHub` therefore never regenerates. Boxes on a different protocol object are skipped, matching the recompile `readParameters` schedules. `RegenerateOnLoad = false` turns it off. Standing proof: `tmp/smoke_test_phaseselector_regenerate.m`.
