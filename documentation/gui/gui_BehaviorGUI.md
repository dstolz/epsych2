...
- `register(comp, name)` — add **any** component built with its native API (`gui.ParameterScatter`, `gui.History`, `gui.PhaseSelector`, a secondary figure, ...) to the teardown registry. The optional `name` is what a component toolbar calls it; without one the tool is labelled from the class name, which is only ambiguous when one GUI holds two components of the same class.
