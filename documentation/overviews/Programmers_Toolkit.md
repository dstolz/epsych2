...
| [`gui.Helper`](../../obj/+gui/@Helper/Helper.m) | Static one-liners you would otherwise re-derive: `dprime2AFC(HR)`, `criterion(HR,FR)`, `percent_correct(HR,FR)`, plus table row highlighting and timed color changes. |
...
|---|---|
| `addControl(parent, param, …)` | A `gui.Parameter_Control` bound to a parameter. **An unresolved name returns `[]` without error**, so one `build` serves protocols with differing parameter sets. |
...
| `addMonitor(parent, params, …)` | A `gui.Parameter_Monitor`. Registered monitors stop polling when the session mode goes to Stop. |
| `addNextTrial(parent, …)` | The upcoming-trial display. |
| `addPerformance(parent, …)` | A `gui.SessionPerformance` computed through this GUI's psychophysics object when it has one, so trial-type conventions match. |
| `addSyringePump(parent, …)` | The `gui.SyringePump` panel over the session's `hw.NE1000` — or a standalone one when the protocol has no pump, so the GUI still opens. |
...
| Component | Shows |
|---|---|
| [`gui.History`](../../obj/+gui/@History/History.m) | Trial-by-trial table, rows colorable by decoded response bit. |
| [`gui.NextTrial`](../../obj/+gui/@NextTrial/NextTrial.m) | Parameters of the trial about to run, refreshed on every `NewTrial`. |
| [`gui.SessionPerformance`](../../obj/+gui/@SessionPerformance/SessionPerformance.m) | Counts, rates, d′, criterion, with the trial window on a right-click menu. |
| [`gui.ParameterScatter`](../../obj/+gui/@ParameterScatter/ParameterScatter.m) | Any per-trial parameter against any other, with an optional color-by parameter, all chosen from dropdowns at runtime. |
| [`gui.PsychPlot`](../../obj/+gui/@PsychPlot/PsychPlot.m) | Online psychometric summary (d′, hit rate, FA rate, bias) driven by a psychophysics object's own `NewData`. |
| [`gui.OnlinePlot`](../../obj/+gui/@OnlinePlot/OnlinePlot.m) | Real-time multi-trace hardware activity for one box, with time window and trial-locked modes. |
| [`gui.SlidingWindowPerformancePlot`](../../obj/+gui/@SlidingWindowPerformancePlot/SlidingWindowPerformancePlot.m) | Performance metrics over a moving trial window. |
| [`gui.Performance`](../../obj/+gui/@Performance/Performance.m) | Summary table of performance metrics from a linked psychophysics object. |
| [`gui.Parameter_Monitor`](../../obj/+gui/@Parameter_Monitor/Parameter_Monitor.m) | Polled current values of an array of parameters. |
| [`gui.SessionClock`](../../obj/+gui/@SessionClock/SessionClock.m) | Up to four live readouts: since last trial, since first trial, since session start, wall clock. |
| [`gui.ElapsedTrialTimer`](../../obj/+gui/@ElapsedTrialTimer/ElapsedTrialTimer.m) | Time since the last completed trial; usable headlessly by reading `ElapsedTime`. |
| [`gui.ModeIndicator`](../../obj/+gui/@ModeIndicator/ModeIndicator.m) | Lamp and label reflecting the current `hw.DeviceState`. |
| [`gui.StatusBar`](../../obj/+gui/@StatusBar/StatusBar.m) | Footer message line, green for success and red for errors; double-click copies the text. |
...
| [`gui.ComponentToolbar`](../../obj/+gui/@ComponentToolbar/ComponentToolbar.m) | One tool per display. **Lazy** entries declared with `addLazyComponent(name, factory, …)` are built on first click, which is how a paradigm offers a display without spending a polling timer or listeners on it up front. |
| [`gui.Parameter_Control`](../../obj/+gui/Parameter_Control.m) | The bound editor itself: edit field, range pair, dropdown, checkbox, button, or `'auto'`. |
| [`gui.Parameter_Update`](../../obj/+gui/Parameter_Update.m) | The commit button controller — enables itself when any watched editor is dirty; Ctrl-click discards. |
...
| [`gui.ScreenCapture`](../../obj/+gui/@ScreenCapture/ScreenCapture.m) | Copy the whole window — controls and plots alike — to the clipboard. Renders offscreen, so an obscured window still copies. |
...
| [`gui.FilenameValidator`](../../obj/+gui/@FilenameValidator/FilenameValidator.m) | Edit field that enforces `.mat`, rejects invalid characters, and warns on an existing file. |
...
| To read a parameter beside a running session | `gui.ParameterDebugger` | `gui.Parameter_Monitor` | The debugger reads only on demand; the monitor polls, which is a cost you should choose deliberately. |
