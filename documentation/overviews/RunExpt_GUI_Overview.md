...
- **View**:
  ...
  - **Psychophysics Metrics...** (`Ctrl+M`) — a map of one signal-detection metric (d′, criterion, ln β, A′, B″, balanced proportion correct) over the whole hit-rate/false-alarm plane, with every metric read out at the point you click and a plain-language explanation of the selected one beside it. Useful for judging whether a session's numbers are plausible, and for seeing what a correction for rates of 0 and 1 actually does to them. It computes nothing of its own — every value is `psychophysics.Metrics` — and it touches no hardware, so it can stay open beside a running session. See [../gui/gui_MetricsExplorer.md](../gui/gui_MetricsExplorer.md).
