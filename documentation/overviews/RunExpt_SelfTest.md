...
**Environment & installation** — repository metadata, that every required function and class resolves on the path, that the stimgen submodule is checked out, that `vprintf` output actually reaches the log file, and that no stale RunExpt windows or orphaned timers are left over from a previous session.
