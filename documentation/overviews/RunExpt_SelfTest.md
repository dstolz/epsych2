...
The **Open Log** button flushes the logger before opening the file, so the run you just finished is on disk and not still in a buffer. The log check (A4) reports which destinations are active, and fails with a specific remedy if the file sink has disabled itself after a write failure — see [granary](../granary/granary_Logging.md).
