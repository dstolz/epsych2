...
**No two consecutive delays share a list value.** A block holds every value once, so a repeat can only happen where one block ends and the next begins — unconstrained, one boundary in `numel(list)`, or about one in seven for `1000:500:4000`. The selector gives the sequence `MaxConsecutive = 1` whenever the list has more than one value, and the rule holds across a mid-session edit too (see [epsych.BlockSequence](../epsych/epsych_BlockSequence.md#editing-the-value-list-mid-session)). It is about list values, not the jittered numbers: with jitter at or above half the step, neighbouring delays from adjacent values can still land close together. Repeat-on-abort, above, is the one deliberate exception.
