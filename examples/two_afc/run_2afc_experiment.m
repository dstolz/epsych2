function [RUNTIME, GUI] = run_2afc_experiment(options)
...
% collected exactly as in a real session.
%
...
% path printed. Analyze it with explore_2afc_data (same folder).
%
...
%
% Options:
%   NumTrials    - Trials to run before auto-stop (default 64, i.e. 8 per condition)
%   SubjectName  - Name recorded in the session data (default 'Me')
%   ProtocolFile - .eprot to run; created if missing (default: TwoAFC.eprot here)
...
%   run_2afc_experiment(NumTrials=24)   % a quick taste
%
...
if ~isfile(options.ProtocolFile)
    create_2afc_protocol(options.ProtocolFile);
end
...
vprintf(0, 'Session running: %d trials. Which lamp is brighter? Answer LEFT or RIGHT.', ...
    options.NumTrials)
vprintf(0, 'The arrow keys answer too. The session saves and stops by itself.')
...
vprintf(0, 'Next: explore_2afc_data(''%s'')', fn)
end
