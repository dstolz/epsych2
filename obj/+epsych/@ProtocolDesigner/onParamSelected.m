...
%	evt	- UITable selection event with row and column indices.
    ...
    if prevCol == 5 && prevRow >= 1 && prevRow <= numel(obj.ParameterHandles)
        obj.TableParams.Data{prevRow, 5} = obj.getParameterValueDisplay(obj.ParameterHandles{prevRow});
    end
    ...
    if col == 5 && row >= 1 && row <= numel(obj.ParameterHandles)
        obj.TableParams.Data{row, 5} = obj.getParameterValueFull(obj.ParameterHandles{row});
    end
