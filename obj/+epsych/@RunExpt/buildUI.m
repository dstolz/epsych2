...
mVideo = uimenu(f,'Label','Video');
uimenu(mVideo,'Label','Video Settings...','Separator','on', ...
    'MenuSelectedFcn', @(~,~) self.DefineVideoSettings);
self.H.mnu_webcam_record = uimenu(mVideo,'Label','Webcam Preview / Record...', ...
    'Accelerator','W', ...
    'MenuSelectedFcn', @(~,~) self.WebcamRecord());
self.H.mnu_webcam_stop = uimenu(mVideo,'Label','Stop Webcam', ...
    'Enable','off', ...
    'MenuSelectedFcn', @(~,~) self.WebcamStop());
