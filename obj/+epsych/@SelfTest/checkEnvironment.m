...
required = [ ...
    ...
    "granary.isEnabled", "granary.Logger", "granary.sink.TextFile", ...
...
fileSink = L.sinkOfType('granary.sink.FileSink');
logPath = L.LogFile;
...
if isempty(fileSink)
    r = epsych.SelfTest.result("A4_Logging", GROUP, "Log file writable", "fail", ...
        ...
        Remedy = "Restore the default logger: granary.Logger.instance('-reset').");
else
    ...
    elseif fileSink.Failed
        r = epsych.SelfTest.result("A4_Logging", GROUP, "Log file writable", "fail", ...
            ...
            Remedy = "Fix write access to the log directory, then clear the latch " + ...
                "with granary.Logger.instance().sinkOfType('granary.sink.FileSink').reset().");
    else
        r = epsych.SelfTest.result("A4_Logging", GROUP, "Log file writable", "fail", ...
...
if isempty(which('stimgen.util.logSink'))
    ...
    if isempty(bridge) || ~isa(bridge, 'stimbridge.LogBridge')
        ...
        logPath6 = granary.Logger.instance().LogFile;
