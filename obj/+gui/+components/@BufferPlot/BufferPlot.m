classdef BufferPlot < gui.PopOut
    ...
    %
    %   obj = gui.components.BufferPlot(source, container)
    %   obj = gui.components.BufferPlot(source, container, Buffers=["Waveform~1"], SampleRate=24414)
    %
    ...
    %   figure (or an explicit PreferenceTag), with the rule gui.components.OnlinePlot
    ...
    %     gui.components.BufferPlot([S.data_0001 S.data_0002], uifigure);
    %
    % Documentation: documentation/gui/gui_BufferPlot.md
    % See also gui.components.OnlinePlot, gui.components.ParameterScatter, gui.BehaviorGUI, gui.PopOut
    ...
    properties (Constant, Access = private)
        ...
        % Okabe-Ito, the colorblind-safe qualitative set gui.components.OnlinePlot uses.
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.BufferPlot.getComponentSpec()
    ...
    methods
        function obj = BufferPlot(source, container, options)
            ...
            %
            % Returns:
            %   obj - gui.components.BufferPlot object.
            arguments
                source = []
                container = []
                options.Buffers = {}
        ...
        function setBuffers(obj, names)
            ...
            T = gui.components.BufferPlot.emptyTrace_;
            for k = 1:numel(names)
                t = obj.resolveTrace_(names{k});
                if isempty(t)
                    vprintf(2,'gui.components.BufferPlot: "%s" is not a buffer this session offers', names{k})
                    continue
                end
                T(end+1) = t;
            end
        ...
        function n = availableBuffers(obj)
            ...
            try
                ...
                vprintf(2,'gui.components.BufferPlot: buffer list unavailable: %s', ME.message)
            end
        ...
        function selectBuffers(obj)
            ...
            if isempty(choices)
                vprintf(0,1,'gui.components.BufferPlot: no buffer parameters are available to plot')
                return
            end
        ...
        function s = exportToWorkspace(obj, varName)
            ...
            if ~isempty(varName)
                ...
                vprintf(1,'gui.components.BufferPlot: %d buffer(s) copied to base workspace as "%s"', ...
        ...
        function forgetConfiguration(obj)
            ...
            try
                ...
                vprintf(2,'gui.components.BufferPlot: failed to clear preferences: %s', ME.message)
            end
        end
    end
    ...
    methods (Access = protected)
        function h = createPopOut_(obj, container)
            ...
            h = gui.components.BufferPlot(obj.Source_, container, ...
                BoxID = obj.BoxID, ...
                PreferenceTag = tag);
    ...
    methods (Access = private)
        function build_(obj, container)
            ...
            % cannot (unlike gui.components.OnlinePlot, which needs a duration ruler).
            try
        ...
        function D = currentData_(obj)
            ...
            % gui.components.ParameterScatter uses: TRIALS(i).DATA is grown by
        ...
        function captureTrial_(obj)
            ...
            for k = 1:numel(obj.Traces_)
                inRecord = false;
                if ~isempty(D)
                    inRecord = ~isempty(gui.components.BufferPlot.recordValue_(D(end), obj.Traces_(k).ValidName));
                end
        ...
        function v = readParameter_(obj, k)
            ...
            try
                ...
                vprintf(2,'gui.components.BufferPlot: "%s" could not be read: %s', T.Name, ME.message)
                return
            end
        ...
        function v = rawValue_(obj, k, key)
            ...
            if T.FromData
                ...
                v = gui.components.BufferPlot.recordValue_(D(j), T.ValidName);
                return
            end
        ...
        function [xi,y] = cachedEnvelope_(obj, k, key, v)
            ...
            [xi,y] = gui.components.BufferPlot.envelope_(v, obj.MaxPoints);
    ...
    methods (Access = private)
            ...
            if ~isempty(obj.Runtime_)
                ...
                if ~isempty(P)
                    ...
                    if ~ismember(P(1).Type, obj.BUFFER_TYPES)
                        ...
                        vprintf(2,'gui.components.BufferPlot: "%s" is a %s parameter, not a buffer', ...
                            name, P(1).Type)
                    end
                    try
                        fs = P(1).Module.Fs;
        ...
        function names = excludedNames_(obj)
            ...
            try
                ...
                vprintf(2,'gui.components.BufferPlot: parameter types unavailable: %s', ME.message)
            end
        end
        ...
        function names = autoSelect_(obj)
            ...
            try
                ...
                vprintf(2,'gui.components.BufferPlot: auto-selection failed: %s', ME.message)
            end
            if numel(names) > obj.MAX_AUTO
                vprintf(2,'gui.components.BufferPlot: %d buffers available; plotting the first %d', ...
        ...
        function tf = applySampleRateOption_(obj, rate)
            ...
            if ~ismember(lower(char(rate)), {'auto','module'})
                vprintf(0,1,'gui.components.BufferPlot: SampleRate must be a number, "auto", or "module"')
                return
            end
            fs = 0;
            for k = 1:numel(obj.Traces_)
            ...
            if fs <= 0
                vprintf(2,'gui.components.BufferPlot: no module reported a sample rate; plotting samples')
                return
            end
            obj.SampleRate = fs;
            tf = true;
            vprintf(2,'gui.components.BufferPlot: sample rate taken from the module: %g Hz', fs)
        end
    ...
    methods (Access = private)
        ...
        function buildContextMenu_(obj)
            ...
            try
                ...
                vprintf(3,'gui.components.BufferPlot: context menu unavailable: %s', ME.message)
            end
        end
        ...
        function promptSampleRate_(obj)
            ...
            if isnan(fs) || fs < 0
                vprintf(0,1,'gui.components.BufferPlot: "%s" is not a sample rate', answer{1})
                return
            end
            obj.SampleRate = fs;
        ...
        function promptYLimits_(obj)
            ...
            if ~isfinite(lo) || ~isfinite(hi) || hi <= lo
                vprintf(0,1,'gui.components.BufferPlot: y limits must be finite and increasing')
                return
            end
            obj.YLim = [lo hi];
            obj.setAesthetic_('YLimMode','manual');
        ...
        function loadPreferences_(obj, hasExplicitBuffers, hasExplicitRate)
            ...
            % gui.components.OnlinePlot settled on:
            ...
            try
                ...
                vprintf(3,'gui.components.BufferPlot: restored saved settings "%s"',pname)
            catch ME
                obj.Suspend_ = wasSuspended;
                vprintf(2,'gui.components.BufferPlot: failed to load preferences: %s', ME.message)
            end
        end
        ...
        function savePreferences_(obj)
            ...
            try
                ...
                vprintf(2,'gui.components.BufferPlot: failed to save preferences: %s', ME.message)
            end
        end
