classdef ComponentToolbar < handle
    ...
    properties (Access = private)
        Entries_ struct = gui.components.ComponentToolbar.emptyEntries_() % one per tool; see blankEntry_
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.ComponentToolbar.getComponentSpec()
    ...
    methods
        ...
        function tool = addLazyComponent(obj, name, factory, options)
            ...
            e            = gui.components.ComponentToolbar.blankEntry_();
        ...
        function delete(obj)
            ...
            for i = 1:numel(obj.Entries_)
                ...
                if e.kind == "lazy"
                    gui.components.ComponentToolbar.teardownWindow_(e.comp, e.fig, e.prefTag);
                end
            end
            obj.Entries_(:) = [];
        end
    ...
    methods (Hidden)
        ...
        function populateAuto_(obj, comps, labels)
            ...
            for i = 1:numel(comps)
                ...
                if obj.isExcluded_(c, lab)
                    vprintf(3, 'gui.components.ComponentToolbar: "%s" excluded', lab)
                    continue
                end
                ...
                e.style     = obj.Style;
                e.comp      = c;
                e.iconName  = gui.components.ComponentToolbar.iconNameForClass(class(c));
                ...
                if e.name ~= lab
                    ...
                    vprintf(2, ['gui.components.ComponentToolbar: "%s" is already on the ' ...
        ...
        function tf = openLazyByName_(obj, name)
            ...
            if ~isempty(obj.Entries_(i).fig) && isvalid(obj.Entries_(i).fig)
                gui.components.ComponentToolbar.raise_(obj.Entries_(i).fig);
                return
            end
            obj.openLazy_(i);
            obj.syncTool_(i);
    ...
    methods (Access = private)
        ...
        function tool = makeTool_(obj, i)
            ...
            args = { ...
                ...
                'Icon',      gui.components.ComponentToolbar.resolveIcon_(e.iconName), ...
            ...
            try
                ...
                if e.kind == "auto"
                    if isempty(e.comp) || ~isvalid(e.comp)
                        vprintf(2, 'gui.components.ComponentToolbar: "%s" is gone; disabling its tool', name)
                    ...
                    if e.style == "toggle" && isOpen
                        obj.closeLazy_(name);
                    elseif isOpen
                        gui.components.ComponentToolbar.raise_(e.fig);
                    else
                        obj.openLazy_(i);
                    end
                end
        ...
        function openLazy_(obj, i)
            ...
            if isempty(h) || ~isa(h, 'handle') || ~isvalid(h)
                vprintf(2, 'gui.components.ComponentToolbar: factory for "%s" made nothing', e.name)
                delete(fig)
                return
            end
            ...
            fig.Visible = 'on';
            vprintf(2, 'gui.components.ComponentToolbar: opened "%s"', fig.Name)
            obj.noteParent_();
        end
        ...
        function noteParent_(obj)
            ...
            try
                ...
                vprintf(3, 'gui.components.ComponentToolbar: could not report a window change: %s', ME.message)
            end
        end
    ...
    methods (Static)
        ...
        function label = entryLabel(cls, registerName)
            % label = gui.components.ComponentToolbar.entryLabel(cls, registerName)
        ...
        function nm = iconNameForClass(cls)
            % nm = gui.components.ComponentToolbar.iconNameForClass(cls)
    ...
    methods (Static, Access = private)
        ...
        function icon = resolveIcon_(name)
            ...
            try
                ...
                vprintf(3, ['gui.components.ComponentToolbar: no "%s" glyph in gui.toolbarIcon; ' ...
