classdef ElapsedTrialTimer < handle
    % gui.components.ElapsedTrialTimer(options)
    % gui.components.ElapsedTrialTimer(parent, options)
    ...
    %
    % Usage:
    %   t = gui.components.ElapsedTrialTimer;                        % standalone, no GUI label
    %   t = gui.components.ElapsedTrialTimer(fig);                   % with label in figure
    %   t = gui.components.ElapsedTrialTimer(fig, FontSize=14, Prefix='ITI: ');
    %   t.attachRuntime(RUNTIME);
    %   t.start;
    %   elapsed = t.ElapsedTime;                          % query programmatically
    %
    % See also: epsych.EventHub, gui.components.ModeIndicator, gui.GenericTimer
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.ElapsedTrialTimer.getComponentSpec()
