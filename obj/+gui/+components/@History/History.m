classdef History < gui.PopOut
    ...
    %
    % Example:
    %   H = gui.components.History(pObj, uifigure);
    %
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.History.getComponentSpec()
    ...
    methods
        ...
        function obj = History(pObj,container,options)
            ...
            %
            % Returns:
            %   obj - gui.components.History object.
            %
        ...
        function build(obj)
            ...
            try
                ...
                vprintf(3,'gui.components.History: column sorting/rearranging unavailable: %s',ME.message)
            end
        ...
        function set.psychObj(obj,pobj)
            ...
            assert(epsych.EventHub.valid_psych_obj(pobj),'gui.components.History:set.psychObj', ...
    ...
    methods (Static, Access = private)
        function tf = isScalarLike(value)
            % tf = gui.components.History.isScalarLike(value)
    ...
    methods (Access = protected)
        ...
        function h = createPopOut_(obj,container)
            ...
            h = gui.components.History(obj.psychObj,container, ...
    ...
    methods (Access = private)
        ...
        function [DataOut,FN,meta] = buildRows(obj,DataIn,trialNumbers,baseline,responseBits)
            ...
            ind = structfun(@(a) ~gui.components.History.isScalarLike(a),DataIn(1));
        ...
        function order = resolveDisplayOrder(obj,RD,columnNames)
            ...
            try
                ...
                vprintf(2,'gui.components.History: unable to sort by "%s": %s',name,ME.message)
                order = (n:-1:1)';
            end
        end
        ...
        function onColumnsRearranged(obj,event)
            ...
            try
                ...
                vprintf(2,'gui.components.History: unable to read rearranged column order: %s',ME.message)
                return
            end
        ...
        function params = availableParameters(obj)
            ...
            ind = cellfun(@(f) gui.components.History.isScalarLike(D(1).(f)), fn);
            params = sort(fn(ind));
        end
        ...
        function buildContextMenu(obj)
            ...
            try
                ...
                vprintf(3,'gui.components.History: context menu unavailable: %s',ME.message)
            end
        end
        ...
        function loadPreferences(obj)
            ...
            try
                ...
                if ispref(obj.PREF_GROUP,pname)
                    ...
                    vprintf(3,'gui.components.History: loaded saved preferences "%s"',pname)
                end
            catch ME
                vprintf(2,'gui.components.History: failed to load preferences: %s',ME.message)
            end
        end
        ...
        function savePreferences(obj)
            ...
            try
                ...
                vprintf(2,'gui.components.History: failed to save preferences: %s',ME.message)
            end
        end
        ...
        function applyPendingPreferences(obj)
            ...
            try
                ...
                vprintf(3,'gui.components.History: applied saved preferences')
            catch ME
                vprintf(2,'gui.components.History: failed to apply saved preferences: %s',ME.message)
            end
        end
