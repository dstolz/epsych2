classdef NextTrial < gui.PopOut
    % obj = gui.components.NextTrial(source, container)
    ...
    % an explicit PreferenceTag), the same pattern gui.components.ParameterScatter and
    % gui.components.Parameter_Monitor use.
    %
    ...
    %
    % See also: documentation/gui/gui_NextTrial.md, gui.BehaviorGUI.add,
    % gui.components.Parameter_Monitor, gui.components.ParameterScatter, epsych.TrialsData
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.NextTrial.getComponentSpec()
    ...
    methods (Access = protected)
        ...
        function h = createPopOut_(obj, container)
            ...
            h = gui.components.NextTrial(obj.Source_, container, ...
                Fields        = obj.SelectedFields_, ...
    ...
    methods (Access = private)
        ...
        function seedFromRuntime_(obj, source)
            ...
            % (gui.components.ParameterScatter backfills DATA the same way.)
            try
                T = source.TRIALS;
        ...
        function s = formatValue_(obj, name, raw)
            if isKey(obj.Formatters_, name)
                try
                    ...
                    vprintf(2,'gui.components.NextTrial: formatter for "%s" failed: %s', name, ME.message)
                end
            end
            s = gui.components.NextTrial.defaultFormat_(raw);
        end
        ...
        function createContextMenu_(obj)
            ...
            try
                ...
                vprintf(3,'gui.components.NextTrial: context menu unavailable: %s', ME.message)
                obj.ContextMenu = [];
                return
            end
            ...
            try
                ...
                try
                    ...
                    vprintf(3,'gui.components.NextTrial: cannot attach context menu: %s', ME.message)
                end
            end
        end
        ...
        function promptFontSize_(obj)
            ...
            try
                ...
                vprintf(0,1,'gui.components.NextTrial: cannot prompt for a font size: %s', ME.message)
                return
            end
            ...
            if ~isfinite(n) || n <= 0
                vprintf(1,'gui.components.NextTrial: "%s" is not a font size', strtrim(a{1}))
                return
            end
            obj.setFontSize(n);
        end
        ...
        function applyFontSize_(obj)
            ...
            try
                ...
                vprintf(3,'gui.components.NextTrial: unable to set font size: %s', ME.message)
            end
        end
        ...
        % ------------------------------------------------------------
        % Preference persistence (mirrors gui.components.ParameterScatter)
        ...
        function loadPreferences_(obj)
            try
                ...
                vprintf(3,'gui.components.NextTrial: loaded saved preferences "%s"', pname)
            catch ME
                vprintf(2,'gui.components.NextTrial: failed to load preferences: %s', ME.message)
            end
        end
        ...
        function savePreferences_(obj)
            try
                ...
                vprintf(2,'gui.components.NextTrial: failed to save preferences: %s', ME.message)
            end
        end
