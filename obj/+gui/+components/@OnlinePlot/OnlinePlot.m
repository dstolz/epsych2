classdef OnlinePlot < gui.PopOut
    ...
    %
    %   obj = gui.components.OnlinePlot(RUNTIME, source, hax, BoxID)
    %   obj = gui.components.OnlinePlot(..., PreferenceTag=tag)
    %
    ...
    %   scheme gui.components.ParameterScatter uses. Two rules keep a remembered layout
    ...
    % See also gui.PopOut, gui.BehaviorGUI, gui.components.ParameterScatter
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.OnlinePlot.getComponentSpec()
            ...
            s.postFcn         = @gui.components.OnlinePlot.applySpecDefaults;
            s.options         = [ ...
    ...
    methods
        ...
        function forgetConfiguration(obj)
            ...
            try
                ...
                vprintf(2,'gui.components.OnlinePlot: failed to clear preferences: %s',ME.message)
            end
        end
    ...
    methods (Access = private)
        ...
        function c = paletteColors_(obj,n)
            ...
            switch lower(obj.palette)
                ...
                otherwise,        P = gui.components.OnlinePlot.PALETTE; % Okabe-Ito
            end
            m = size(P,1);
            c = zeros(n,3);
            for i = 1:n
    ...
    methods (Access = protected)
        function resolve_source(obj,source)
            ...
            if isempty(source)
                ...
                if isempty(p)
                    vprintf(0,1,'gui.components.OnlinePlot: this runtime has no readable parameters to plot')
                    return
                end
        ...
        function v = sample_values(obj)
            ...
            for k = 1:numel(obj.localRows_)
                try
                    raw(obj.localRows_(k)) = gui.components.OnlinePlot.scalarize_(obj.localParams_(k).Value);
                catch ME
            ...
            for g = 1:numel(obj.readGroups_)
                ...
                if G.Batched
                    try
                        ...
                        for k = 1:numel(G.Rows)
                            raw(G.Rows(k)) = gui.components.OnlinePlot.scalarize_(val{k});
                        end
                        continue
                    catch ME
                ...
                for k = 1:numel(G.Rows)
                    try
                        raw(G.Rows(k)) = gui.components.OnlinePlot.scalarize_(G.Params(k).Value);
                    catch ME
        ...
        function P = readableParameters_(obj,seed)
            ...
            try
                ...
                vprintf(3,'gui.components.OnlinePlot: could not list readable parameters: %s',ME.message)
            end
        end
