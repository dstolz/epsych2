...
%
% See also: gui.components.OnlinePlot.reorderTraces, gui.components.OnlinePlot.traceNames
...
if isnumeric(order) || islogical(order)
    ...
    if ~isequal(sort(perm), 1:n)
        vprintf(0,1,'gui.components.OnlinePlot: setTraceOrder needs a permutation of 1:%d',n)
        return
    end
else
