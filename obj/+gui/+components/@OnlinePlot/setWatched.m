...
%             samples for nothing.
%
% See also: gui.components.OnlinePlot.setTraceOrder, gui.components.OnlinePlot.selectTraces
...
if isempty(source)
    vprintf(1,'gui.components.OnlinePlot: ignoring an empty trace selection')
    return
end
...
if isempty(obj.BM) && isempty(obj.watchedParams)
    vprintf(0,1,'gui.components.OnlinePlot: nothing matched the requested traces; leaving the plot unchanged')
    return
end
