...
%
%   Construction
%     M = gui.components.Parameter_Monitor(parent)
%     M = gui.components.Parameter_Monitor(parent, Parameters)
%     M = gui.components.Parameter_Monitor(parent, Parameters, pollPeriod=..., type=...)
%     M = gui.components.Parameter_Monitor(parent, Parameters, type="graphical", ...
...
%     M = gui.components.Parameter_Monitor(f, p, pollPeriod=0.5, type="table");
%
%     % Graphical dashboard: lamps for state monitors, labels elsewhere
%     M = gui.components.Parameter_Monitor(f, p, type="graphical", LayoutColumns=2, ...
...
% See also gui.components.Parameter_Control, gui.components.Parameter_Update, timer, uitable, uilamp
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.Parameter_Monitor.getComponentSpec()
    ...
    methods
        ...
        function obj = Parameter_Monitor(parent,Parameters,options)
            ...
            if obj.type ~= "table" && ~isempty(obj.Columns)
                vprintf(2,'gui.components.Parameter_Monitor: Columns option is ignored for type="%s"',obj.type)
            end
            ...
            if ~isempty(parent)
                ...
                try
                    ...
                    vprintf(2,'gui.components.Parameter_Monitor: initial poll failed: %s',ME.message)
                end
        ...
        function add_parameter(obj, parameter)
            ...
            for p = parameter
                if any(obj.Parameters == p)
                    vprintf(1,'gui.components.Parameter_Monitor: parameter "%s" is already monitored',p.Name)
                    continue
                end
        ...
        function set_parameter_visible(obj, name, tf)
            ...
            for n = name
                ...
                if isempty(hit)
                    vprintf(1,'gui.components.Parameter_Monitor: "%s" is not monitored',n)
                    continue
                end
                for k = hit
        ...
        function set_parameter_color(obj, name, options)
            ...
            if isempty(p)
                vprintf(1,'gui.components.Parameter_Monitor: "%s" is not monitored',name)
                return
            end
    ...
    methods (Access = protected)
        ...
        function h = createPopOut_(obj, container)
            ...
            h = gui.components.Parameter_Monitor(container, obj.Parameters, ...
    ...
    methods (Access = private)
            ...
            switch obj.type
                ...
                case "table"
                    ...
                    try
                        ...
                        vprintf(3,'gui.components.Parameter_Monitor: column sort/rearrange unavailable: %s',ME.message)
                    end
        ...
        function create_timer(obj)
            ...
            tname = sprintf('Parameter_Monitor_Timer_%d', gui.components.Parameter_Monitor.next_instance_id_());
        ...
        function timer_tick_(obj)
            ...
            try
                ...
                vprintf(2,'gui.components.Parameter_Monitor: poll failed: %s',ME.message)
            end
        end
        ...
        function refresh_after_rebuild_(obj)
            ...
            try
                ...
                vprintf(2,'gui.components.Parameter_Monitor: refresh failed: %s',ME.message)
            end
        end
        ...
        function apply_font_size_(obj, h)
            ...
            try
                ...
                vprintf(3,'gui.components.Parameter_Monitor: unable to set font size: %s',ME.message)
            end
        end
        ...
        function style = resolve_style_(obj, p)
            ...
            if ~ismember(style, gui.components.Parameter_Monitor.SUPPORTED_STYLES)
                vprintf(2,'gui.components.Parameter_Monitor: unknown style "%s" for "%s"; using "label"',style,p.Name)
                style = "label";
            end
            ...
            if style == "gauge" && ~(isfinite(p.Min) && isfinite(p.Max))
                vprintf(2,'gui.components.Parameter_Monitor: "%s" needs finite Min/Max for a gauge; using "label"',p.Name)
                style = "label";
            end
        end
        ...
        function pick_color_(obj, p, field)
            ...
            try
                ...
                vprintf(2,'gui.components.Parameter_Monitor: color picker unavailable: %s',ME.message)
                return
            end
        ...
        function order = resolve_sort_order_(obj,columnNames,N,V,rawExtra)
            ...
            try
                ...
                vprintf(2,'gui.components.Parameter_Monitor: unable to sort by "%s": %s',name,ME.message)
                order = (1:n)';
            end
        end
        ...
        function update_column_widths_(obj,columnNames,data)
            ...
            try
                ...
                vprintf(3,'gui.components.Parameter_Monitor: unable to set column widths: %s',ME.message)
            end
        end
        ...
        function create_context_menu_(obj)
            ...
            try
                ...
                vprintf(3,'gui.components.Parameter_Monitor: context menu unavailable: %s',ME.message)
                obj.ContextMenu = [];
                return
            end
            ...
            for k = 1:numel(targets)
                ...
                try
                    ...
                    try
                        ...
                        vprintf(3,'gui.components.Parameter_Monitor: cannot attach context menu: %s',ME.message)
                    end
                end
            end
        end
        ...
        function idx = resolve_menu_target_(obj,evt)
            ...
            try
                ...
                vprintf(3,'gui.components.Parameter_Monitor: cannot resolve right-click target: %s',ME.message)
            end
        end
        ...
        function load_preferences(obj)
            ...
            try
                ...
                vprintf(3,'gui.components.Parameter_Monitor: loaded saved preferences "%s"',pname)
            catch ME
                vprintf(2,'gui.components.Parameter_Monitor: failed to load preferences: %s',ME.message)
            end
        end
        ...
        function save_preferences(obj)
            ...
            try
                ...
                vprintf(2,'gui.components.Parameter_Monitor: failed to save preferences: %s',ME.message)
            end
        end
