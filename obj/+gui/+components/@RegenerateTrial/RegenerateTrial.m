classdef RegenerateTrial < handle
    ...
    % can make. The combination is the one gui.components.Parameter_Update already uses
    ...
    properties (Constant)
        ...
        % arbitrary choice: gui.components.Parameter_Update already uses exactly this
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.RegenerateTrial.getComponentSpec()
    ...
    methods
        ...
        function obj = RegenerateTrial(RUNTIME, parent, options)
            % obj = gui.components.RegenerateTrial(RUNTIME, parent, ...)
            ...
            if isempty(tooltip), tooltip = gui.components.RegenerateTrial.DEFAULT_TOOLTIP; end
        ...
        function attachRuntime(obj, RUNTIME)
            ...
            if ~isa(RUNTIME, 'epsych.Runtime') || ~isvalid(RUNTIME)
                vprintf(2, 'gui.components.RegenerateTrial: no session supplied; the button is inert')
                obj.applyEnable_();
                return
            end
    ...
    methods (Access = private)
        ...
        function setArmed_(obj, mods)
            tf = all(ismember(gui.components.RegenerateTrial.ARM_MODIFIERS, mods));
        ...
        function tf = canRegenerate_(obj)
            ...
            if obj.RequireArming && ~obj.Armed
                vprintf(1, ['gui.components.RegenerateTrial: ignored -- hold Ctrl+Alt+Shift to arm ' ...
