classdef ScreenCapture < handle
    % gui.components.ScreenCapture(parent, options)
    ...
    %
    % Usage:
    %   gui.components.ScreenCapture(fig);                        % icon-only button
    %   gui.components.ScreenCapture(g, Text='Screenshot');       % in a grid cell, labeled
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.ScreenCapture.getComponentSpec()
    ...
    methods
        ...
        function tf = copyToClipboard(obj)
            ...
            if isempty(obj.Target) || ~all(isvalid(obj.Target))
                vprintf(0, 1, 'gui.components.ScreenCapture: no figure to capture')
            ...
            tmp = [tempname '.png'];
            cleaner = onCleanup(@() gui.components.ScreenCapture.deleteQuietly_(tmp));

            try
                if ispc
                    exportapp(obj.Target, tmp);
                    gui.components.ScreenCapture.loadAssemblies_();
                    ...
                    vprintf(1, ['gui.components.ScreenCapture: copied the plots only; the ' ...
                        'full window, controls included, is a Windows-only copy'])
                end

                tf = true;
                vprintf(1, sprintf('gui.components.ScreenCapture: copied "%s" to the clipboard', ...
                    obj.Target.Name))
