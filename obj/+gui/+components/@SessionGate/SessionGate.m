classdef SessionGate < handle
    ...
    %   gui.components.SessionGate is the arming control for a rig that must not start
    ...
    % See also gui.BehaviorGUI, gui.components.ModeIndicator, epsych.RunExpt
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.SessionGate.getComponentSpec()
    ...
    methods
        function obj = SessionGate(parent, options)
            % obj = gui.components.SessionGate(parent, Name=Value)
        ...
        function release(obj)
            ...
            vprintf(1, 'gui.components.SessionGate: operator began the experiment')
