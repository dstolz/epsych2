classdef SyringePump < gui.PopOut
    % obj = gui.components.SyringePump(source, container, Name=Value)
    ...
    properties
        ...
        Sections (1,:) string = gui.components.SyringePump.SECTIONS
    end
    ...
    methods (Static)
        function s = getComponentSpec()
            % s = gui.components.SyringePump.getComponentSpec()
    ...
    methods
        ...
        function obj = SyringePump(source, container, options)
            % obj = gui.components.SyringePump(source, container, ...)
            ...
            obj.seedSetting_('RateUnits', ...
                char(gui.components.SyringePump.pick_(options, saved, 'RateUnits', 'MM', true)), 'MM');
            obj.seedSetting_('VolumeUnits', ...
                char(gui.components.SyringePump.pick_(options, saved, 'VolumeUnits', 'mL', true)), 'mL');
            obj.Applying_ = false;

            % Layout: the caller's Sections is the default the menu resets
            % to, and a saved selection overrides it (as gui.components.NextTrial's
            % saved field selection overrides its Fields option).
            obj.DefaultSections_ = obj.normalizeSections_( ...
                gui.components.SyringePump.pick_(options, saved, 'Sections', "All", false));
            obj.Sections = gui.components.SyringePump.pick_(options, saved, 'Sections', ...
            ...
            obj.Applying_ = true;
            obj.seedSetting_('Diameter', gui.components.SyringePump.pick_(options, saved, 'Diameter', 21.59, true), 21.59);
            obj.seedSetting_('Rate',     obj.initialRate_(options, saved), 0.7);
            obj.seedSetting_('Direction', ...
                char(gui.components.SyringePump.pick_(options, saved, 'Direction', 'Infuse', true)), 'Infuse');
            obj.seedSetting_('TTLTrigger', ...
                logical(gui.components.SyringePump.pick_(options, saved, 'TTLTrigger', false, true)), false);
            obj.Applying_ = false;
            ...
            obj.seedSetting_('TriggerMode', ...
                char(gui.components.SyringePump.pick_(options, saved, 'TriggerMode', ...
        ...
        function delete(obj)
            ...
            try
                ...
                vprintf(2, 'gui.components.SyringePump: releasing the interface failed: %s', ME.message)
            end
        end
        ...
        function connect(obj)
            ...
            if isempty(obj.StagedPort_)
                ...
                vprintf(0, 1, 'gui.components.SyringePump: choose a serial port before connecting')
                return
            end
        ...
        function port = detectPort(obj)
            ...
            if isempty(port)
                ...
                vprintf(0, 1, 'gui.components.SyringePump: no NE-1000 answered on any available serial port')
                if wasConnected
                    obj.connect();
                end
                return
            ...
            vprintf(1, 'gui.components.SyringePump: found a pump on %s', port)
    ...
    methods (Access = protected)
        ...
        function h = createPopOut_(obj, container)
            ...
            h = gui.components.SyringePump(obj.Interface, container, ...
    ...
    methods (Access = private)
        ...
        function resolveInterface_(obj, source)
            ...
            if ~isempty(source) && isobject(source) && isprop(source, 'Interfaces')
                try
                    ...
                    vprintf(2, 'gui.components.SyringePump: could not search the runtime interfaces: %s', ME.message)
                end
            end

            if ~isempty(source) && ~isa(source, 'hw.NE1000') && ~isobject(source)
                vprintf(2, 'gui.components.SyringePump: unrecognized source; using a standalone interface')
            end
        ...
        function iface = ensureInterface_(obj)
            ...
            vprintf(2, 'gui.components.SyringePump: no pump interface supplied; created a standalone one')
        end
        ...
        function seedSetting_(obj, name, value, default)
            ...
            try
                ...
                vprintf(1, ['gui.components.SyringePump: remembered %s is not usable (%s); ' ...
        ...
        function tf = requireLink_(obj)
            ...
            if ~tf
                ...
                vprintf(1, 'gui.components.SyringePump: no pump connected')
            end
        end
        ...
        function applyRateUnits_(obj)
            ...
            vprintf(1, ['gui.components.SyringePump: switching %s rate units from %s to %s ' ...
            ...
            try
                ...
                vprintf(3, 'gui.components.SyringePump: could not relabel the Rate parameter: %s', ME.message)
            end
        end
        ...
        function onRateUnitsChanged_(obj, wasUnits)
            ...
            if obj.IsConnected && ismember(obj.Status, obj.RUNNING_STATES)
                vprintf(0, 1, ['gui.components.SyringePump: the pump is %s; stop it before ' ...
            ...
            obj.Applying_ = true;
            obj.Rate = gui.components.SyringePump.convertRate_(obj.Rate, wasUnits, obj.RateUnits);
            obj.Applying_ = false;
        ...
        function warnRateResolution_(obj)
            ...
            if obj.Rate >= 10000
                vprintf(0, 1, ['gui.components.SyringePump: %.6g %s is more than the pump can be ' ...
                    'told (its commands carry 4 digits); choose a coarser unit'], ...
                    obj.Rate, obj.rateLabel_())
            elseif obj.Rate < 0.01
                vprintf(0, 1, ['gui.components.SyringePump: %.4g %s is finer than the pump can be ' ...
        ...
        function readFromHardware_(obj)
            ...
            try
                ...
                vprintf(2, 'gui.components.SyringePump: reading the pump settings failed: %s', ME.message)
            end
            obj.Applying_ = false;
        ...
        function createTimer_(obj)
            tname = sprintf('SyringePump_Timer_%d', gui.components.SyringePump.nextInstanceId_());
        ...
        function timerTick_(obj)
            ...
            try
                ...
                vprintf(2, 'gui.components.SyringePump: poll failed: %s', ME.message)
            end
        end
        ...
        function names = normalizeSections_(obj, value)
            ...
            for v = value
                ...
                if isempty(k)
                    vprintf(1, ['gui.components.SyringePump: "%s" is not a section of this panel; ' ...
        ...
        function renderVolume_(obj)
            ...
            s = gui.components.SyringePump.formatVolume_(v);
        ...
        function r = initialRate_(obj, options, saved)
            ...
            r = gui.components.SyringePump.pick_(options, saved, 'Rate', 0.7, true);
            ...
            r = gui.components.SyringePump.convertRate_(r, was, obj.RateUnits);
        end
        ...
        function createContextMenu_(obj)
            ...
            try
                ...
                vprintf(3, 'gui.components.SyringePump: context menu unavailable: %s', ME.message)
                obj.ContextMenu = [];
                return
            end
            ...
            for k = 1:numel(targets)
                ...
                try
                    ...
                    vprintf(3, 'gui.components.SyringePump: cannot attach context menu: %s', ME.message)
                end
            end
        end
        ...
        function refreshValueMenu_(obj)
            ...
            tm = uimenu(m, Text = sprintf('TTL Trigger (%s, %s)', ...
                gui.components.SyringePump.ttlLabel_(obj.TTLTrigger), obj.TriggerMode));
            for tf = [true false]
                item = uimenu(tm, Text = gui.components.SyringePump.ttlLabel_(tf), ...
        ...
        function promptValue_(obj, name)
            ...
            try
                ...
                vprintf(0, 1, 'gui.components.SyringePump: cannot prompt for %s: %s', name, ME.message)
                return
            end
            ...
            if ~isfinite(v)
                vprintf(0, 1, 'gui.components.SyringePump: "%s" is not a number', strtrim(a{1}))
                return
            end
        ...
        function s = loadPreferences_(obj)
            ...
            try
                ...
                vprintf(3, 'gui.components.SyringePump: loaded saved configuration "%s"', pname)
            catch ME
                vprintf(2, 'gui.components.SyringePump: failed to load preferences: %s', ME.message)
                s = struct();
            end
        end
        ...
        function savePreferences_(obj)
            try
                ...
                vprintf(2, 'gui.components.SyringePump: failed to save preferences: %s', ME.message)
            end
        end
    ...
    methods (Static, Access = private)
        ...
        function v = convertRate_(v, from, to)
            ...
            v = double(v) * gui.components.SyringePump.rateFactor_(from) ...
                          / gui.components.SyringePump.rateFactor_(to);
        end
