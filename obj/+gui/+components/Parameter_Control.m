classdef Parameter_Control < handle & matlab.mixin.SetGet
    ...
    %   gui.components.Parameter_Control creates a labeled UI component that displays
    ...
    %   immediately.
    %
    %   OBJ = gui.components.Parameter_Control(PARENT, PARAMETER) creates a numeric edit
    %   field for PARAMETER inside PARENT.
    %
    %   OBJ = gui.components.Parameter_Control(PARENT, PARAMETER, Type=TYPE,
    ...
    %
    %       gui.components.Parameter_Control(layout, p, Type='range', autoCommit=true, ...
    ...
    %
    %       hCatch = gui.components.Parameter_Control(layout, pCatchEnabled, Type='checkbox');
    %       gui.components.Parameter_Control(layout, pCatchRate, EnabledBy=hCatch);
    %
    ...
    % See also gui.components.Parameter_Monitor, gui.components.Parameter_Update
    ...
    methods (Static)
        function specs = getComponentSpec()
            % specs = gui.components.Parameter_Control.getComponentSpec()
            ...
            common.className       = 'gui.components.Parameter_Control';
            ...
            ctrl.postFcn        = @gui.components.Parameter_Control.applyControlDefaults;
        ...
        function opts = applyButtonType(opts, ~, ~)
            ...
            % momentary press.
            %
            % This runs BEFORE construction because gui.components.Parameter_Control's
        ...
        function applyButtonStyle(h, guiObj, ctx)
            ...
            % the +1.
            n = numel(gui.components.Parameter_Control.buttonsIn(guiObj)) + 1;
        ...
        function c = buttonsIn(guiObj)
            % c = gui.components.Parameter_Control.buttonsIn(behaviorGUI)
            ...
            % momentary or toggle.
            c = guiObj.componentsOfClass('gui.components.Parameter_Control');
    ...
    methods
        ...
        function obj = Parameter_Control(parent,Parameter,options)
            arguments
                ...
                % a gui.components.Parameter_Control -- normally a checkbox or toggle
        ...
        function addDependent(obj, ctrl, mode)
            ...
            % apply it now.
            %  ctrl - gui.components.Parameter_Control to govern. [] is ignored, so a
            ...
            if isempty(ctrl) || ~isa(ctrl,'gui.components.Parameter_Control') || ~isvalid(ctrl)
                return
            end
    ...
    methods (Access = private)
        function logCommit_(obj, prevValue, newValue)
            ...
            % or not the GUI shows a gui.components.Notes component. Runtime is empty for
            ...
            try
                ...
                vprintf(3, 'gui.components.Parameter_Control: commit not recorded for "%s" (%s)', ...
                    obj.Name, ME.message)
            end
        end
                ...
                case 'editfield'
                    ...
                    if isempty(v) || (~isnumeric(v) && ~islogical(v))
                        ...
                        elseif v < lims(1) || v > lims(2)
                            vprintf(2, ['gui.components.Parameter_Control: "%s" holds %g, outside its ' ...
