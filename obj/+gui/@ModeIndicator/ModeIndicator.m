classdef ModeIndicator < handle
    % gui.ModeIndicator(parent, Name=Value)
    ...
    %
    % Usage:
    %   ind = gui.ModeIndicator(fig);
    %   ind = gui.ModeIndicator(gridCell, FontSize=13);
    ...
    methods
        ...
        function setState(obj, mode)
            ...
            [color, label] = gui.ModeIndicator.lookupState_(mode);
    ...
    methods (Static, Access = private)
        ...
        function [color, label] = lookupState_(mode)
            ...
            map = gui.ModeIndicator.STATE_MAP_;
            key = int32(mode);
            if isKey(map, key)
                entry = map(key);
                color = entry{1};
