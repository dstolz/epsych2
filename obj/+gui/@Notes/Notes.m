classdef Notes < gui.PopOut
    % obj = gui.Notes(source, container)
    ...
    % keeps them too. Several gui.Notes over one store -- an embedded one and
    ...
    methods
        ...
        function setEditable(obj, tf)
            ...
            if obj.ReviewMode_ && tf
                vprintf(1, 'gui.Notes: a reviewed session''s notes are a record and cannot be edited')
                return
            end
            obj.Editable_ = tf;
            obj.applyEditable_();
        ...
        function clearNotes(obj)
            ...
            try
                ...
                vprintf(2, 'gui.Notes: could not confirm the clear (%s)', ME.message)
                answer = 'Cancel';
            end
    ...
    methods (Access = protected)
        ...
        function h = createPopOut_(obj, container)
            ...
            h = gui.Notes(obj.Store, container, ...
    ...
    methods (Access = private)
        ...
        function resolveStore_(obj, source)
            ...
            vprintf(2, 'gui.Notes: no session supplied; these notes are not attached to a data file')
        ...
        function createContextMenu_(obj)
            ...
            try
                ...
                vprintf(3, 'gui.Notes: context menu unavailable: %s', ME.message)
                obj.ContextMenu = [];
                return
            end
            ...
            for h = [obj.LogH, obj.EntryH, obj.CommitH]
                try
                    ...
                    vprintf(3, 'gui.Notes: could not attach the context menu (%s)', ME.message)
                end
            end
        end
        ...
        function copyAll_(obj)
            ...
            try
                ...
                vprintf(2, 'gui.Notes: copied %d line(s) to the clipboard', numel(obj.LogH.Value))
            catch ME
                vprintf(0, 1, ME)
            end
        end
        ...
        function loadPreferences_(obj)
            ...
            try
                ...
                if ispref(gui.Notes.PREF_GROUP, key)
                    P = getpref(gui.Notes.PREF_GROUP, key);
                    if isstruct(P) && isfield(P, 'Editable')
                        obj.Editable_ = logical(P.Editable);
                    end
                end
            catch ME
                vprintf(3, 'gui.Notes: could not read saved settings (%s)', ME.message)
            end
        end

        function savePreferences_(obj)
            try
                setpref(gui.Notes.PREF_GROUP, obj.preferenceTag_(), ...
                    struct('Editable', obj.Editable_));
            catch ME
                vprintf(3, 'gui.Notes: could not save settings (%s)', ME.message)
            end
        end
    end
end
