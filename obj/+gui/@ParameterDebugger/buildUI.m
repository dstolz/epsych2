...
self.H.mnu_track = uimenu(mParams, 'Text','&Track Selected', 'Accelerator','T', ...
    'Separator','on', 'MenuSelectedFcn', @(~,~) self.trackSelected());
self.H.mnu_track_new = uimenu(mParams, 'Text','Track Selected in &New Plot', ...
    'MenuSelectedFcn', @(~,~) self.trackSelected(NewPlot = true));
...
% ---------- Source bar --------------------------------------------------
% Column 9 is an empty spacer, not a button: giving the flexible width to
% "Read Selected" instead is how that button ends up clipped to "Read Selecte"
% on a narrow window.
gTop = uigridlayout(g, [1 11]);
gTop.Layout.Row = 1;
gTop.RowHeight = {24};
gTop.ColumnWidth = {50, 200, 110, 120, 40, 180, 26, 70, '1x', 120, 110};
...
% Filtering is live: ValueChangingFcn fires on every keystroke, ValueChanged
% only on Enter or focus loss. Both are wired because the second is what a
% paste or a programmatic set arrives on.
self.H.filter = uieditfield(gTop, 'text', ...
    'Placeholder','name, module, or interface  (Ctrl+F)', ...
    'Tooltip', ['Show only parameters whose interface, module, or name matches. ' ...
                'Filters as you type; Esc clears it. Tick Regex to match with a ' ...
                'regular expression instead of a substring.'], ...
    'ValueChangingFcn', @(~,evt) self.onFilterTyped_(evt.Value), ...
    'ValueChangedFcn', @(src,~) self.onFilterTyped_(src.Value));
self.H.filter.Layout.Column = 6;

self.H.btnClear = uibutton(gTop, 'Text','X', ...
    'Tooltip','Clear the Find box (Esc)', ...
    'ButtonPushedFcn', @(~,~) self.clearFilter_());
self.H.btnClear.Layout.Column = 7;

% Regex is off by default: a plain substring is what a person types when they
% want "everything with Freq in the name", and the characters a regex reads
% specially (. ( ) [ ) are common in parameter names.
self.H.chkRegex = uicheckbox(gTop, 'Text','Regex', 'Value', false, ...
    'Tooltip', ['Read the Find text as a case-insensitive regular expression ' ...
                'matched against "interface / module name".'], ...
    'ValueChangedFcn', @(~,~) self.onFilterTyped_(self.H.filter.Value));
self.H.chkRegex.Layout.Column = 8;
...
self.H.btnReadSel.Layout.Column = 10;
...
self.H.btnReadAll.Layout.Column = 11;
...
self.H.table = uitable(g, ...
    ...
    'Tooltip', ['Double-click a parameter name to read it. Type into the Value ' ...
                'column to write it. Click a header to sort; drag one to move ' ...
                'the column.'], ...
...
% Sorting and rearranging are the display's business only. Everything here
% turns a row back into an hw.Parameter -- and getting that translation wrong
% means writing to the wrong parameter on live hardware -- so every callback
% uses the DATA index uitable reports (Selection, evt.Indices,
% InteractionInformation.Row), never the Display* counterpart the header click
% reorders. Sorting is lexicographic on the displayed text, because every
% column is char; the natural order (interface, then module, then declaration
% order) is what the list is built in and what clearing the sort returns to.
...
self.H.cmnu_fire = uimenu(cm, 'Text','Fire Trigger', ...
    'MenuSelectedFcn', @(~,~) self.fireTrigger());
self.H.cmnu_track = uimenu(cm, 'Text','Track Selected (Ctrl+T)', ...
    'MenuSelectedFcn', @(~,~) self.trackSelected());
...
uilabel(gLegend, 'Text','Value:', 'FontColor',[0.35 0.38 0.42], ...
    'Tooltip', ['A Value cell is tinted by the last read or write of that ' ...
        'parameter. An uncoloured cell has never been asked.']);
localSwatch(gLegend, self.COLOR_OK,    'read', ...
    'Read from the source; the cell shows what the device reported.');
localSwatch(gLegend, self.COLOR_WROTE, 'written', ...
    'Written from this window, and the read-back confirmed it.');
localSwatch(gLegend, self.COLOR_STALE, 'differs', ...
    ['Written, but reads back different -- clamping, an Expression, or a ' ...
        'coarse device. Not an error: the cell shows what the device holds.']);
localSwatch(gLegend, self.COLOR_FAIL,  'failed', ...
    'The read or write threw; the Note column carries the message.');
localSwatch(gLegend, self.COLOR_SKIP,  'not read', ...
    ['Skipped by Read All: write-only, or a buffer left on the device. ' ...
        'Double-click the name to read it anyway.']);
...
% -----------------------------------------------------------------------
function localSwatch(parent, color, text, tip)
% One legend entry: a tinted label carrying its own meaning as its text, and
% the fuller explanation as a tooltip.
