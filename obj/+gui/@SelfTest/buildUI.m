...
self.H.optBehaviorGUI = uicheckbox(gTop, ...
    'Text', 'Launch the Behavior GUI', ...
    'Value', self.Engine.IncludeBehaviorGUI, ...
    'Tooltip', ['Launch the configured behavior GUI against a synthetic runtime and close it.' newline ...
                'ep_GenericGUI allows only one instance, so an open behavior GUI will be replaced.'], ...
    'ValueChangedFcn', @(~,~) self.onOptionChanged);
self.H.optBehaviorGUI.Layout.Row = 1; self.H.optBehaviorGUI.Layout.Column = 2;
