...
if ~isempty(rx) && isvalid(rx) && rx.STATE >= PRGMSTATE.RUNNING
    if self.Engine.IncludeHardwareConnect || self.Engine.IncludeBoxFig || self.Engine.IncludeGuiStateCycle
        uialert(self.H.figure, ...
            ['A session is running. Checks that connect hardware, launch the box GUI, ' ...
