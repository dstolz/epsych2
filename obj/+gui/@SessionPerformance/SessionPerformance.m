classdef SessionPerformance < gui.PopOut
    ...
    methods (Access = protected)
        ...
        function h = createPopOut_(obj, container)
            ...
            h = gui.SessionPerformance(src, container, ...
    ...
    methods (Access = private)
        ...
        function createContextMenu_(obj)
            ...
            try
                ...
                vprintf(3,'gui.SessionPerformance: context menu unavailable: %s', ME.message)
                obj.ContextMenu = [];
            end
        end
        ...
        function promptCount_(obj, mode)
            ...
            try
                ...
                vprintf(0,1,'gui.SessionPerformance: %s', ME.message)
            end
        end
        ...
        function answer = askUser_(~, title, prompt, dflt)
            ...
            try
                ...
                vprintf(0,1,'gui.SessionPerformance: cannot prompt for a trial window: %s', ME.message)
                return
            end
        ...
        function copySummary_(obj)
            try
                ...
                vprintf(2,'gui.SessionPerformance: summary copied to the clipboard')
            catch ME
                vprintf(0,1,ME)
            end
        end
        ...
        function names = validateMetrics_(~, names)
            ...
            if any(bad)
                vprintf(1,'gui.SessionPerformance: ignoring unknown metric(s) %s', ...
                    strjoin(names(bad), ', '))
                names = names(~bad);
            end
            if isempty(names)
        ...
        % -- Preference persistence (mirrors gui.NextTrial) ----------------
