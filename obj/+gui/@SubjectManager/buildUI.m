...
self.H.tb_copy_project = uipushtool(tb, ...
    'Tag','tb_copy_project', ...
    'Icon',gui.toolbarIcon("projectcopy"), ...
    'Tooltip','Copy Project...: a new project with these settings, with or without its subjects', ...
    'ClickedCallback', @(~,~) self.onCopyProject_());

self.H.tb_edit_project = uipushtool(tb, ...
    'Tag','tb_edit_project', ...
    'Icon',gui.toolbarIcon("projectedit"), ...
    'Tooltip','Edit Project...: name, notes, default protocol, data path, behavior GUI', ...
...
self.H.tb_add_to_session = uipushtool(tb, ...
    ...
    'Tooltip',['Add Checked to Session: the checked subjects become the session''s ' ...
        'whole subject list, replacing whoever is in it now'], ...
...
% Session settings live on the subject's membership in the selected project,
% so this needs a project context; updateEnableStates_ gates it.
self.H.mnu_edit_membership = uimenu(mSubject, 'Text','&Session Settings...', ...
...
self.H.mnu_copy_project = uimenu(mProject, 'Text','&Copy Project...', ...
    'MenuSelectedFcn', @(~,~) self.onCopyProject_());
% The template is stamped at join and never propagates on its own; this is
% the deliberate push, and the mismatch refusal's named fix.
self.H.mnu_reapply_template = uimenu(mProject, 'Text','Re-&apply Project Template to Checked', ...
    'Separator','on', ...
    'MenuSelectedFcn', @(~,~) self.onReapplyTemplate_());
...
gProjBtns = uigridlayout(gLeft, [1 4]);
gProjBtns.Layout.Row = 3;
gProjBtns.ColumnWidth = {'1x','1x','1x','1x'};
...
self.H.btnCopyProject = uibutton(gProjBtns, 'Text','Copy...', ...
    'Tooltip','A new project with this one''s settings, with or without its subjects', ...
    'ButtonPushedFcn', @(~,~) self.onCopyProject_());
...
% Settings says whether the row's membership still matches the project's
% session-defaults template ('template') or has been edited per-subject
% ('edited') -- which is what makes the commit-time mismatch refusal
% predictable before the click.
self.H.table = uitable(gRight, ...
    'ColumnName', {'', 'Subject', 'Box', 'Protocol', 'Version', 'Settings', ...
                   'Species', 'Sex', 'Weight', 'Last Run', 'Status'}, ...
    'ColumnFormat', {'logical', 'char', 'numeric', 'char', 'char', 'char', 'char', ...
                     'char', 'numeric', 'char', 'char'}, ...
    'ColumnEditable', [true false true false false false false false false false false], ...
    'ColumnWidth', {30, 140, 50, 170, 110, 65, 90, 70, 60, 130, 70}, ...
    'RowName', {}, ...
    'RowStriping','on', ...
    'SelectionType','row', ...
...
% Tooltipped because the button replaces rather than appends: the operator has
% to be able to learn that without first watching a subject disappear.
self.H.btnAddToSession = uibutton(gActions, 'Text','Add Checked to Session', ...
    'FontWeight','bold', ...
    'BackgroundColor',[0.20 0.52 0.85], 'FontColor',[1 1 1], ...
    'Tooltip',['The checked subjects become the session''s whole subject list, ' ...
        'replacing whoever is in it now'], ...
