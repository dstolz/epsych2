...
for i = 1:8
    specs(end + 1) = local_spec_(sprintf('Port%dIn', i), false, ...
        struct('Type', 'Boolean', 'Access', 'Read', 'Visible', false, ...
               'Description', sprintf("Port %d IR beam. True matches whatever raises the Port%dIn event on the device (firmware raises it on a LOW->HIGH edge), so this read and the trial event stream always agree.", i, i)));
end
...
% Worth recording rather than merely logging: the firmware stops storing
% timestamps past MAX_TIMESTAMPS while the state machine keeps running and keeps
% reporting events, so on a long or noisy trial the counts legitimately diverge
% and the timestamps are truncated. Without this flag in DATA there is no way to
% tell such a trial from a clean one after the session.
specs(end + 1) = local_spec_('EventCountMismatch', false, ...
    struct('Type', 'Boolean', 'Access', 'Read', 'Visible', true, ...
           'Description', "True when the decoded event count disagreed with the epilogue header, i.e. the trial's timestamps are incomplete."));
