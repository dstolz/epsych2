...
% See also: stimgen.LogSink, stimgen.util.logSink, granary.Logger, vprintf
    ...
    methods
        function emit(~,level,red,msg,args)
            ...
            %
            %   * cache the logger in a property. granary.Logger.instance('-reset')
            ...
            %   * call the vprintf facade. It would re-run granary.isEnabled,
            %     which stimgen has already run, and add a stack frame that
            %     granary.callerFrame would then have to skip.
            ...
            granary.Logger.instance().emit(level,logical(red),msg,args);
        end

        function tf = isEnabled(~,level)
            % tf = isEnabled(obj,level)
            % Defer to EPsych's gate so granary.isEnabled is the single reader of
            ...
            tf = granary.isEnabled(level,'any');
        end
    end
end
