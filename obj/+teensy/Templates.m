classdef Templates
    ...
    methods (Static, Access = private)
        ...
        function p = twoAFC_()
            % p = twoAFC_()
            % Center poke initiates; a left or right choice is recorded.
            % The board reports the Choice_* bit; correctness against
            % @TrialType is the host's to score (psychophysics.NAFC).
            p = teensy.Program(Name = "TwoAlternativeForcedChoice", ...
                Description = "Center initiation, then a left or right choice recorded as Choice_0/Choice_1.");
            ...
            % The board records WHAT was chosen and nothing more. It cannot
            % compare the choice with @TrialType — the condition language
            % has no variable comparison — so it must not claim a Hit;
            % psychophysics.NAFC scores correctness from the Choice bit
            % against the correct alternative, which is exactly the case
            % this leaves to the host.
            p.addState(teensy.State("ChoseLeft", IsTerminal = true, ...
                RespCodeBits = [epsych.BitMask.Choice_0, epsych.BitMask.Reward], ...
                Notes = "Chose left. Correct when TrialType is 0; the host scores it."));
            p.addState(teensy.State("ChoseRight", IsTerminal = true, ...
                RespCodeBits = [epsych.BitMask.Choice_1, epsych.BitMask.Reward], ...
                Notes = "Chose right. Correct when TrialType is 1; the host scores it."));
            p.addState(teensy.State("NoChoice", IsTerminal = true, ...
                Notes = "The choice window closed with no response: Undefined, no outcome bit."));
            p.addState(teensy.State("Abort", IsTerminal = true, ...
                RespCodeBits = epsych.BitMask.Abort, ...
