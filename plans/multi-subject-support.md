...
- **Configuration layer**: `RunExpt.CONFIG` is a `(1,:)` per-subject struct array (`obj/+epsych/@RunExpt/RunExpt.m:22`). `AddSubject` supports up to 16 boxes with unique BoxIDs and a distinct protocol file per subject; `RemoveSubject`, `SortBoxes`, per-subject `EditProtocol`/`UpdateProtocol`/`ChangeProtocolFile` context actions all index the selected subject. `.ecfg` save/load round-trips the whole array. SelfTest check D5 explicitly guards multi-subject CONFIG capacity.
