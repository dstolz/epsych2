...
assert(numel(g.NotesButton.OpenH.Parent.ColumnWidth) == 8, ...
    'the button row should have columns for Notes and Regenerate beside the six triggers');
assert(isvalid(g.RegenerateButton) && isa(g.RegenerateButton,'gui.RegenerateTrial'), ...
    'regenerate button should exist');
assert(g.RegenerateButton.ButtonH.Parent == g.NotesButton.OpenH.Parent, ...
    'regenerate button should share the button row with the triggers and Notes');
