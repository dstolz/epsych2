...
% Exercise gui.ComponentToolbar and gui.BehaviorGUI.addComponentToolbar.
%
...
assert(isa(tb,'gui.ComponentToolbar'), 'addComponentToolbar should return a gui.ComponentToolbar');
...
expect = { ...
    'gui.ParameterScatter',    "Parameter Scatter",   "parameterscatter"; ...
    'gui.History',             "History",             "history"; ...
    'gui.SessionPerformance',  "Session Performance", "sessionperformance"; ...
    'gui.NextTrial',           "Next Trial",          "nexttrial"; ...
    'gui.Parameter_Monitor',   "Parameter Monitor",   "parametermonitor"; ...
    'gui.PsychPlot',           "Psych Plot",          "psychplot"; ...
    'gui.SyringePump',         "Syringe Pump",        "syringepump"; ...
    'psychophysics.Staircase', "Staircase",           "staircase"};
for k = 1:size(expect,1)
    cls = expect{k,1};
    assert(gui.ComponentToolbar.entryLabel(cls) == expect{k,2}, ...
        '%s should be labelled "%s", got "%s"', cls, expect{k,2}, ...
        gui.ComponentToolbar.entryLabel(cls));
    assert(gui.ComponentToolbar.iconNameForClass(cls) == expect{k,3}, ...
        '%s should use icon "%s", got "%s"', cls, expect{k,3}, ...
        gui.ComponentToolbar.iconNameForClass(cls));
    gui.toolbarIcon(expect{k,3}); % every adopter has a glyph of its own
end
assert(gui.ComponentToolbar.entryLabel('gui.ParameterScatter','Left Box') == "Left Box", ...
...
% Tools are tagged with the entry name, exactly as gui.ComponentToolbar builds it.
